...
def analyze_file(input_file: typing.TextIO,
    ...
    compile_code = CompilationEngine(tokens, output_file)
    tokens = compile_code.tokenizer
    while tokens.has_more_tokens():
        token_type = tokens.token_type()
        a = tokens.input_tokens[tokens.ind]
        if token_type == "STRING_CONST":
            str_const = tokens.string_val()
            compile_code.out.write("\t<stringConstant> " + str_const + "</stringConstant>\n")
        elif token_type == "SYMBOL":
            sym = tokens.symbol()
            compile_code.out.write("\t<symbol> " + sym + " </symbol>\n")
        elif token_type == "INT_CONST":
            int_const = tokens.int_val()
            compile_code.out.write("\t<integerConstant> " + str(int_const) + " </integerConstant>\n")
        elif token_type == "IDENTIFIER":
            name = tokens.identifier()
            compile_code.out.write("\t<identifier> " + name + " </identifier>\n")
        elif token_type == "KEYWORD":
            key = tokens.keyword()
            if key == "CLASS":
                compile_code.compile_class()
            elif key in ["LET", "DO", "WHILE", "IF", "RETURN"]:
                compile_code.compile_statements()
            elif key in ["STATIC", "FIELD"]:
                compile_code.compile_class_var_dec()
            elif key in ["INT", "CHAR", "BOOLEAN", "VAR", "VOID"]:
                compile_code.compile_var_dec()
            elif key in ["FUNCTION", "METHOD", "CONSTRUCTOR"]:
                compile_code.compile_subroutine()
            elif key in ["TRUE", "FALSE", "THIS", "NULL"]:
                compile_code.out.write("<keyword> " + key.lower() + " </keyword>\n")
        tokens.advance()
